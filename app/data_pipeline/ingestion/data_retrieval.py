...
def chunk_text(text, chunk_size=250):
    ...
    while start < len(text):
        ...
        match = sentence_end_pattern.search(text[start:end])
        if match:
            end = start + match.end()
        else:
...
def combine_chunks(chunks, group_size=5):
    combined_chunks = []
    
    for i in range(0, len(chunks), group_size):
        combined_chunk = ' '.join(chunks[i:i+group_size])
...
for bill in bills:
    ...
    combined_chunks = combine_chunks(chunked_text, group_size=5)
